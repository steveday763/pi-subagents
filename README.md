...
- **FleetView** — Claude Code-style navigable list of `main` + every running subagent rendered below the editor (earliest-launched first). Press `↓` (or `←`) at an empty prompt to jump in, `↑`/`↓` to move the selection, and `Enter` to switch Pi's native transcript, editor and footer to that agent. The normal editor then steers a running agent or resumes a finished one; select `main` to switch the same native surface back. Toggle via `/agents → Settings → Fleet view`
...
Requires pi **0.84.2 or 0.84.3**. FleetView's native session switch is a version-pinned `InteractiveMode` integration because pi does not yet expose a public live-session viewport API. An unsupported ABI fails explicitly; there is no ConversationViewer fallback. The `peerDependencies` range declares the same two verified versions.
...
Running [workflows](#subagentworkflow) appear as a single `workflow` row above the agents, carrying their agent counts in place of a description. `Enter` on one opens the same two-pane inspector `/agents → Workflows` does. A run's own agents are *not* listed separately — they belong to the run, which reports for them, so they are filtered out of the fleet list, the above-editor widget, the `/agents` menus and `@handle` resolution exactly as nested children are. They are also outside the `maxConcurrent` pool: the run has its own concurrency cap, and routing a fan-out through the session pool as well would let one workflow starve everything else. The agents are ordered earliest-launched first, and only agents with a live session are shown (pending agents appear once their session exists). At an **empty prompt**, press `↓` (or `←`) to move focus from the prompt into the list — the selected row is marked `●`, the rest `○`. The selected row renders in the theme's primary text color rather than the muted/dim treatment of the others; an agent with a configured `color` shows its badge there too, bolded. `↑`/`↓` move the selection and `Enter` switches Pi's native transcript, editor, footer, event subscription and autocomplete source to the selected agent. The normal editor steers it while running; submitting after completion resumes that same session through the ordinary background-agent lifecycle. Select `main` and press `Enter` to restore the root session. Commands that replace the root runtime (`/new`, `/resume`, `/fork`, `/clone`, `/tree`, `/import`, `/reload`) and `/agents` are rejected in a child view with a prompt to switch to `main` first. A completed agent remains listed while selected so its final output stays readable. A switch is refused while the current or target session is compacting, automatically retrying, or running a `!` shell command; wait for that operation to settle and retry, which keeps Pi's operation-specific Escape handler and pending UI state attached to the right session. Typing anything at a non-empty prompt behaves normally — the list only captures arrow keys when the prompt is empty. Disabling FleetView first restores `main` so the user cannot be stranded in a child session; if that restore is temporarily unsafe for the same reason, FleetView remains enabled and shows a warning.
...
src/
  ...
  ui/
    ...
    fleet-list.ts         # FleetView: native main/subagent session switcher below the editor
    native-session-switcher.ts # Version-pinned InteractiveMode transcript/editor bridge
    conversation-viewer.ts # Explicit /agents and workflow inspection overlay
